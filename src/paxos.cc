...
bool proposer::run(int instance, std::vector<std::string> cur_nodes,
    ...
    if (prepare(instance, accepts, cur_nodes, v)) {
        if (majority(cur_nodes, accepts)) {
            tprintf(
                "paxos::manager: received a majority of prepare responses\n");
            ...
            if (majority(cur_nodes, accepts)) {
                tprintf(
                    "paxos::manager: received a majority of accept "
                    "responses\n");
...
bool proposer::prepare(unsigned instance, std::vector<std::string> &accepts,
    ...
    prop_t n_a_max;
    paxos_protocol::status stat;
    size_t  i;
    bool first = true;
    paxos_protocol::prepareres response;
    for (i = 0; i < nodes.size(); i++) {
        handle hdl(nodes[i]);
        rpcc *cl = hdl.safebind();
        if (!cl) {
            continue;
        }

        stat = cl->call(paxos_protocol::preparereq, me, arg, response, rpcc::to(TIMEOUT));
        if (stat == paxos_protocol::OK) {
            ...
            if (response.n_a > n_a_max || first) {
                first = false;
                v = response.v_a;
                n_a_max = response.n_a;
            }
        }
    }
...
void proposer::accept(unsigned instance, std::vector<std::string> &accepts,
    ...
    for (int i = 0; i < nodes.size(); i++) {
        ...
        r = cl->call(paxos_protocol::acceptreq, me, arg, accept,
                     rpcc::to(TIMEOUT));
        if (r == paxos_protocol::OK && accept) {
            accepts.push_back(nodes[i]);
            accept = false;
        }
    }
...
void proposer::decide(unsigned instance, std::vector<std::string> accepts,
    ...
    for (size_t i = 0; i < accepts.size(); i++) {
        handle hdl(accepts[i]);
        rpcc *cl = hdl.safebind();
        if (!cl) {
            continue;
        }

        r = cl->call(paxos_protocol::decidereq, me, arg, ret, rpcc::to(TIMEOUT));
