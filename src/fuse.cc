...
yfs_client::status getattr(yfs_client::inum inum, struct stat &st)
{
    ...
    extent_protocol::type_t file_type = yfs->get_file_type(inum);
    if(file_type == extent_protocol::T_FILE) {
        ...
        printf("   getattr -> %llu\n", info.size);
    } else if (file_type == extent_protocol::T_SYMLINK) {
        yfs_client::symlink_info info;
        ret = yfs->get_link(inum, info);
        if (ret != yfs_client::OK)
            return ret;
        ...
        printf("   getattr -> %llu\n", info.size);
    } else if (file_type == extent_protocol::T_DIR) {
...
void fuseserver_symlink(fuse_req_t req, const char *src_file,  fuse_ino_t parent, const char *dest) {
    yfs_client::inum ino;
    int r;
    r = yfs->create_symlink(parent, src_file, dest, ino);
    if ( r != yfs_client::OK) {
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    std::string link_buf;
    if (yfs->read_symlink(ino, link_buf) != yfs_client::OK) {
        fuse_reply_err(req, EIO);
    }

    fuse_reply_readlink(req, link_buf.c_str());
...
void install_signal(int sig, void (*handler)(int)) {
    if (signal(sig, handler) == SIG_ERR) {
        char msg[128];
        sprintf(msg, "error installing signal handler for %s", strsignal(sig));
        perror(msg);
        exit(1);
    }
}
